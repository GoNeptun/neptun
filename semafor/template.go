...
b, err := ioutil.ReadFile(dir) // just pass the file name
	...
	if err :=	t.ExecuteTemplate(&tpl, "main", data);
	err != nil {
		sf.SetErrorLog(err.Error())
		return template.HTML("")
	}
...
n := sf.LoadContentDirectory()
//dir := n + "layouts/template/"
index_dir := n + "layouts/"
	//var content = path.Join(dir, page)
	var index_page = path.Join(index_dir, "index.html")
...
//templates, err := template.New("main").Funcs(funcMap).ParseFiles(pages ...)
...
//t, err := template.New("main").Funcs(funcMap).ParseFiles(index_page)

	//templates, err := template.New("main").ParseFiles(xheader, xmenu, index_page, content, xfooter)
...
if err :=	t.ExecuteTemplate(w, "main", data);
err != nil {
	sf.SetErrorLog(err.Error())
	http.Error(w, "500 Internal Server Error", 500)
	return
